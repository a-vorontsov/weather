when isMainModule:
  import httpclient, times, os, strformat, json, strutils, nap, re, colorize,
      sequtils, table, math, rdstdin, util, distros
  ...
  proc displayCurrentWeather(input: JsonNode) =
    ...
    table.addRow(@["Weather:",
                 weather[0]["description"].getStr().fgWhite.bold])
    table.addRow(@["Temperature:",
                 tempColour("●") & " " &
                 tempColour(fmt"{round(temp)}°C")])
    table.addRow(@["Feels Like:",
                 feels_likeColour("●") & " " &
                 feels_likeColour(fmt"{round(feelsLike)}°C")])
    table.addRow(@["Min | Max:",
                 minColour(fmt"{round(tempMin)}°C") & " | " &
                 maxColour(fmt"{round(tempMax)}°C")])
    table.addRow(@["Wind Speed:",
                 windSpeed.fgWhite.bold & "m/s"])
    table.addRow(@["Humidity:",
                 humidity.fgWhite.bold & "%"])

    table.printTable(disableColour)

  proc formatWeather(input: JsonNode,
                     table: ref AsciiTable,
  ...
  proc main() =
    ...
    add_example(title = "Usage",
                content = "weather [<location|forecast>] [<forecast>]")
    ...
    let location = use_arg(name = "location",
                           kind = "argument",
                           required = false,
                           value = "here",
                           help = "City name to get weather forecast. Use 'here' to get weather for current location. Enclose city names with spaces in \"\"")

    let forecast = use_arg(name = "forecast",
                           kind = "argument",
                           required = false,
                           value = "now",
                           help = "Type of weather forecast to get. Options are: [now, today, tomorrow, week]")

    let version = use_arg(name = "version",
                          kind = "flag",
                          required = false,
                          help = "Display version",
                          alt = "v")

    let allowPerms = use_arg(name = "allow",
                             kind = "flag",
                             required = false,
                             help = "Allow location permissions",
                             alt = "y")

    let help = use_arg(name = "help",
                       kind = "flag",
                       required = false,
                       help = "Display help",
                       alt = "h")

    let noColour = use_arg(name = "no-colour",
                           kind = "flag",
                           required = false,
